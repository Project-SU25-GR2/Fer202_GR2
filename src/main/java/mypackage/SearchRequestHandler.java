...
public class SearchRequestHandler extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			String searchTerm = req.getParameter("searchTerm"); // Lấy giá trị trực tiếp

			// Sử dụng PreparedStatement để tránh SQL Injection và xử lý encoding
			String sql = "SELECT * FROM users WHERE (fname LIKE ? OR lname LIKE ?) AND NOT unique_id=?";

			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, "%" + searchTerm + "%");
			pstmt.setString(2, "%" + searchTerm + "%");
			pstmt.setString(3, session.getAttribute("unique_id").toString());
			...
			while (set.next()) {
				...
				output += "<a href=\"chats?user_id=" + set.getString("unique_id") + "\">\n"
						...
						+ "						<div class=\"details\">\n"
						+ "							<span>" + set.getString("fname") + " " + set.getString("lname")
						+ "</span>\n"
						+ "							<p>" + msg + "</p>\n"
						+ "						</div>\n"
						+ "					</div>\n"
	...
	private ResultSet getMessagesResultSet(String incoming_id, String outgoing_id, Connection conn)
			throws SQLException {
