...
inputField.addEventListener('keydown', function(event) {
  if (event.key === 'Enter' || event.keyCode === 13) {
   ...
   function submitForm() {
			...
			// msg = msg.replaceAll(" ", "__5oO84a9__"); // Loại bỏ replace này

            // Dữ liệu cần gửi trong body
            var data = "outgoing_id=" + encodeURIComponent(out_id) +
                       "&incoming_id=" + encodeURIComponent(in_id) +
                       "&message=" + encodeURIComponent(msg);

            // Send the form data using Ajax
            var xhr = new XMLHttpRequest();
            var url = "insertChat"; // URL Servlet
            ...
            xhr.send(data); // Gửi dữ liệu trong body
